...
let getknownpeers () =
  let cnt = ref 0 in
  let potentialpeers = ref [] in
  let currtm = Int64.of_float (Unix.time()) in
  Hashtbl.iter (fun n lasttm -> if !cnt < 1000 && Int64.sub currtm lasttm < 604800L then (incr cnt; potentialpeers := n::!potentialpeers)) knownpeers;
  !potentialpeers
